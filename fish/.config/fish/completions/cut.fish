# cut
# Autogenerated from man page /usr/local/opt/coreutils/libexec/gnuman/man1/cut.1
complete -c cut -l bytes -d 'select only these bytes'
complete -c cut -l characters -d 'select only these characters'
complete -c cut -l delimiter -d 'use DELIM instead of TAB for field delimiter'
complete -c cut -l fields -d 'select only these fields;  also print any line that contains no delimiter cha…'
complete -c cut -l complement -d 'complement the set of selected bytes, characters or fields'
complete -c cut -l only-delimited -d 'do not print lines not containing delimiters'
