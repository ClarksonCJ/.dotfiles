# unexpand
# Autogenerated from man page /usr/local/opt/coreutils/libexec/gnuman/man1/unexpand.1
complete -c unexpand -l all -d 'convert all blanks, instead of just initial blanks'
complete -c unexpand -l first-only -d 'convert only leading sequences of blanks (overrides -a)'
complete -c unexpand -l tabs -d 'have tabs N characters apart instead of 8 (enables -a)'
complete -c unexpand -l help -d 'display this help and exit'
complete -c unexpand -l version -d 'output version information and exit AUTHOR Written by David MacKenzie'

