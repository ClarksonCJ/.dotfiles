# cp
# Autogenerated from man page /usr/local/opt/coreutils/libexec/gnuman/man1/cp.1
complete -c cp -l archive -d 'same as -dR --preserve=\\,all\\/'
...
complete -c cp -l force -d 'if an existing destination file cannot be opened, remove it and try again (th…'
complete -c cp -l interactive -d 'prompt before overwrite (overrides a previous -n option)'
complete -c cp -l link -d 'hard link files instead of copying'
complete -c cp -l dereference -d 'always follow symbolic links in SOURCE'
complete -c cp -l no-clobber -d 'do not overwrite an existing file (overrides a -u or previous -i option)'
complete -c cp -l no-dereference -d 'never follow symbolic links in SOURCE'
...
complete -c cp -s r -l recursive -d 'copy directories recursively'
...
complete -c cp -l symbolic-link -d 'make symbolic links instead of copying'
complete -c cp -l suffix -d 'override the usual backup suffix'
...
complete -c cp -l verbose -d 'explain what is being done'
complete -c cp -l one-file-system -d 'stay on this file system'
